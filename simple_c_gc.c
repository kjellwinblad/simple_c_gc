...
static unsigned int scgc_hash_key(void * keyPtr){
  /* From https://lemire.me/blog/2018/08/15/fast-strongly-universal-64-bit-hashing-everywhere*/
...
static void* scgc_min(void* a, void* b){
  return a <= b ? a : b;
}

static void* scgc_max(void* a, void* b){
  return a > b ? a : b;
}
...
static void ycf_find_stack_bottom_and_mark_conservative_helper() {
  ...
  scgc_mark_reachable_objects_in_region(scgc_min(scgc_stack_top,
                                                 (void*)&stack_bottom),
                                        scgc_max((void*)&stack_bottom,
                                                 scgc_stack_top));
}
...
static void scgc_mark_reachable_objects_in_region(void* start, void* end){
  ...
  while(iter <= iter_end){
    ...
    if(ref != NULL &&
      ...
      scgc_mark_reachable_objects_in_region(&object->data[0],
                                            &((char*)object->data)[object->size-1]);
    }
    iter = iter + 1;
  }
}
...
static void scgc_collect_unmarked_traverser(size_t index, void* v, void* context){
...
static void scgc_remove_unmarked_objects(){
  ...
  void** objects_to_remove =
    scgc_malloc(sizeof(void*) * nr_of_candidates);
  ch_set_traverse(scgc_objects,
                  scgc_collect_unmarked_traverser,
                  objects_to_remove);
...
static void scgc_unmark_traverser(size_t index,
                                  void* v,
                                  void* context){
  scgc_object_ref* ref = v;
  ref->base->state = scgc_blank_state;
...
int scgc_start_gced_code(int (*main)(int, char *[]),
                         ...
                         void (*my_free)( void* )) {
  volatile int noinline = 1;
  int (*next)(int (*)(int, char *[]),
              int,
              char **[]) =
    (noinline
     ? scgc_start_gced_code_2
     : (  int (*)(int (*)(int, char *[]),
                  int,
                  char **[]))(NULL));
  volatile char** my_argv = (volatile char**)argv;
  ...
  if(my_argv == NULL){
    fprintf(stderr, "scgc_start_gced_code: the argv parameter should not be NULL!");
    exit(1);
  }
