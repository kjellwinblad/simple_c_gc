...
void ch_set_increase_size(ChainedHashSet * set){
    ...
    if(set->size > set->expandTreshold){
        ...
        SortedListSetNode ** newBuckets = set->malloc(sizeof(SortedListSetNode *)*newNumberOfBuckets);
...
void ch_set_initialize(ChainedHashSet * set,
    ...
    set->size = 0;
    set->buckets = set->malloc(sizeof(SortedListSetNode *)*INITIAL_NUMBER_OF_BUCKETS);
...
static inline
ChainedHashSet * ch_set_create(unsigned int keyPosition,
                               void * (*extract_key)(void * v, int keyPos),
                               unsigned int (*hash_key)(void * k),
                               bool (*are_equal)(void * v1, void * v2),
                               char * (*to_string)(void * v1),
...
static inline
void ch_set_traverse(void * setParam,
                     void (*traverser)(size_t index, void* v, void* context), void* context){
    ChainedHashSet * set = (ChainedHashSet*)setParam;
...
char * ch_set_to_string(void * setParam){
    ...
    char * resultString = set->malloc(totalBucketsStringSize + numberOfBuckets*3 - 3 + 3);
    resultString[0] = '[';
